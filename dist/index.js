...
function deviceMatch({
  ...
  function objectMatch(incoming, stored) {
    ...
    if (!stored) {
      return numOfMismatches = stored === incoming ? numOfMismatches : numOfMismatches + 1;
    }
    ...
    for (let i = 0; i < propKeys.length; i++) {
      ...
      if (isPrimitive(storedVal)) {
        numOfMismatches = storedVal === incomingVal ? numOfMismatches : numOfMismatches + 1;
  ...
  function arrayMatch(incoming, stored) {
    for (let i = 0; i < stored.length; i++) {
      ...
      if (isPrimitive(storedVal)) {
        numOfMismatches = storedVal === incomingVal ? numOfMismatches : numOfMismatches + 1;
      } else if (Array.isArray(stored[i])) {
        arrayMatch(incomingVal, storedVal);
      } else {
        objectMatch(incomingVal, storedVal);
      }
